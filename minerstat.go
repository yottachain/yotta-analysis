...
func (analyser *Analyser) TrackingStat(ctx context.Context) {
	entry := log.WithFields(log.Fields{Function: "TrackingStat"})
	urls := analyser.minerStat.AllSyncURLs
	snCount := len(urls)
	collectionMiner := analyser.analysisdbClient.Database(AnalysisDB).Collection(NodeTab)
	collectionSPNode := analyser.analysisdbClient.Database(AnalysisDB).Collection(SpotCheckNodeTab)
	collectionProgress := analyser.analysisdbClient.Database(AnalysisDB).Collection(TrackProgressTab)
	for i := 0; i < snCount; i++ {
		snID := int32(i)
		go func() {
			...
			for {
				...
				if err != nil {
					if err == mongo.ErrNoDocuments {
						...
						if err != nil {
							...
							time.Sleep(time.Duration(analyser.minerStat.WaitTime) * time.Second)
							continue
						}
					} else {
				...
				minerLogs, err := GetMinerLogs(analyser.httpCli, analyser.minerStat.AllSyncURLs[snID], record.Start, analyser.minerStat.BatchSize, time.Now().Unix()-int64(analyser.minerStat.SkipTime))
				if err != nil {
					time.Sleep(time.Duration(analyser.minerStat.WaitTime) * time.Second)
					continue
				}
				for _, item := range minerLogs.MinerLogs {
					if item.Type == DELETE && item.ToStatus == -1 {
						...
						_, err = collectionSPNode.DeleteOne(ctx, bson.M{"_id": item.MinerID})
						if err != nil {
							entry.WithError(err).Errorf("delete spotcheck miner %d", item.MinerID)
						} else {
							entry.Infof("spotcheck miner %d has been deleted", item.MinerID)
						}
					}
				}
				if minerLogs.More {
